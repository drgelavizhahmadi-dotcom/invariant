#!/usr/bin/env python3
"""
Cross-Region Validation (Split C) for DLR Paper - InvariantPIKAN v2 Version
...
Splits:
- C.1: Train on Vietnam → Test on US
- C.2: Train on US → Test on Vietnam  
- C.3: Within-region baselines (VN_only, US_only)
...
class CrossRegionConfig:
    ...
    OUTPUT_DIR = "cross_region_pikan_results"
    
    # InvariantPIKAN v2 architecture
    MODEL_CONFIG = {
        'input_dim': 3,  # temperature, wind_speed, solar_irradiance
        'hidden_dim': 64,
    ...
    LINE_PHYSICS_CONFIG = LinePhysicsConfig(
        num_lines=100,  # Max lines we'll encounter
        reg_weight=0.01,
        init_resistance_factor_mean=0.0,  # log(1.0)
...
# =============================================================================
# Model with LinePhysicsParams
# =============================================================================

class InvariantPIKANWithLinePhysics(nn.Module):
    """
    InvariantPIKAN v2 with LinePhysicsParams for per-line physics.
    """
    
    def __init__(self, model_config: dict, line_physics_config: LinePhysicsConfig):
        super().__init__()
        
        # Base InvariantPIKAN v2 model
    ...
    def register_lines(self, line_ids: List):
        ...
        for line_id in line_ids:
            if line_id not in self.line_id_to_idx:
                self.line_id_to_idx[line_id] = self.next_idx
                self.next_idx += 1
                if self.next_idx >= self.line_physics.num_lines:
                    # Reuse index 0 (global mean) if we run out
                    self.line_id_to_idx[line_id] = 0
    
    def get_line_indices(self, line_ids: torch.Tensor) -> torch.Tensor:
        """Convert line IDs to indices, using 0 (global mean) for unseen lines"""
        indices = []
    ...
    def forward(self, x, weather_dict, line_ids=None, use_global_mean=False):
        """
        Forward pass with per-line physics.
        
        Args:
            x: Input features [batch, input_dim]
            weather_dict: Dict with weather tensors
            line_ids: Line identifiers [batch] (optional)
            use_global_mean: If True, use global mean μ for all lines (for cross-region)
        ...
        if use_global_mean or line_ids is None:
            ...
            # Create dummy line_ids = 0 to get global params via forward
            device = x.device
        ...
        # Add physics params to outputs
        outputs['physics_params'] = params
        
        return outputs
    
    def get_global_physics_params(self):
        """Get global mean physics parameters (for diagnostics)"""
...
class CrossRegionDataset(Dataset):
    ...
    def __init__(
        ...
        # Extract line_ids if available
        if 'line_id' in self.df.columns:
            # Handle NaN and convert to int
            line_ids_raw = self.df['line_id'].fillna(0).values
            self.line_ids = np.array([int(float(x)) if pd.notna(x) else 0 for x in line_ids_raw])
        else:
        ...
        print(f"  {region} dataset: {len(self)} samples, {len(feature_cols)} features, {len(self.unique_lines)} lines")
...
def train_model(train_dataset, val_dataset, config, experiment_name):
    """Train InvariantPIKAN with LinePhysics"""
    ...
    # Create model with LinePhysics
    model = InvariantPIKANWithLinePhysics(config.MODEL_CONFIG, config.LINE_PHYSICS_CONFIG)
    
    # Register training lines
    ...
    print(f"\n📊 Learned Global Physics Parameters:")
    print(f"   Resistance factor μ: {physics_params['resistance_factor']:.4f}")
    print(f"   Emissivity μ: {physics_params['emissivity']:.4f}")
    print(f"   Absorptivity μ: {physics_params['absorptivity']:.4f}")
...
def evaluate_model(model, test_dataset, config, experiment_name, use_global_mean=True):
    """
    Evaluate model on test set.
    
    For cross-region: use_global_mean=True (zero deviations)
    For within-region: use_global_mean=False (use learned per-line params)
    """
    ...
    with torch.no_grad():
        for batch_x, batch_y, batch_line_ids in test_loader:
            ...
            all_temps.extend(T_amb)
            all_wind.extend(wind_speed)
...
def run_experiment(split_type, config, df):
    ...
    use_global_mean = is_cross_region  # Use global mean for cross-region
    ...
    torch.save({
        ...
        'metrics': {k: v for k, v in metrics.items() 
                   if k not in ['predictions', 'targets', 'temperatures', 'wind_speeds']}
...
def create_results_table(all_metrics):
    ...
    table = "## Cross-Region Transfer Learning Results (InvariantPIKAN v2)\n\n"
    ...
    for m in all_metrics:
        ...
        if train == test:
            vs_str = "Baseline"
        elif test == 'VN' and vn_baseline_mae:
            vs_str = f"+{(m['mae'] / vn_baseline_mae - 1) * 100:.1f}%"
        elif test == 'US' and us_baseline_mae:
...
def main():
    parser = argparse.ArgumentParser(description='Cross-Region Validation with InvariantPIKAN v2')
    parser.add_argument('--experiments', nargs='+', 
                       choices=['VN_to_US', 'US_to_VN', 'VN_only', 'US_only', 'all'],
                       default=['all'])
    parser.add_argument('--epochs', type=int, default=50)
    parser.add_argument('--output-dir', type=str, default='cross_region_pikan_results')
    ...
    print("="*70)
    print("CROSS-REGION VALIDATION - INVARIANTPIKAN V2 WITH LINE PHYSICS")
    print("="*70)
    print(f"Device: {config.DEVICE}")
    print(f"Epochs: {config.EPOCHS}")
